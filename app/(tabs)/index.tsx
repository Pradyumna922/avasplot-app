...
// Global cache so insights persist across re-renders
...
function PropertyCard({
  ...
  useEffect(() => {
    ...
    const fetchInsight = async () => {
      // Check cache first
      const cached = insightCache.get(property.$id);
      if (cached) {
        setInsight(cached);
      ...
      try {
        const result = await groqService.generatePropertyInsight(property);
        if (!cancelled && result) {
          insightCache.set(property.$id, result);
          setInsight(result);
        }
      } catch (err) {
            ...
            <View style={styles.insightContent}>
              ...
              <Text style={styles.insightText}>
                {insightLoading
                  ...
                  : `"${insight || 'Premium asset with strong growth potential. Verified for hassle-free ownership.'}"`
                }
              </Text>
            </View>
          </View>
          <Text style={styles.aiDisclaimer}>AI-generated insights may not be accurate. Verify independently before making decisions.</Text>
        </View>
...
const howItWorksSteps = [
  { step: '1', title: 'Apply', desc: 'Submit your onboarding application' },
...
export default function HomeScreen() {
  // Main Tabs State
  const topTabs = ['Plots', 'Services', 'Onboard'];
  const [mainTab, setMainTab] = useState<'Plots' | 'Services' | 'Onboard'>('Plots');
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
          ...
          <LinearGradient colors={['#0F172A', '#1E293B']} style={styles.franchiseHero}>
            <View style={styles.franchiseBadge}>
              <Text style={styles.franchiseBadgeTxt}>ONBOARD PARTNER PROGRAM</Text>
            </View>
            <Text style={styles.franchiseTitle}>Join the Platform.</Text>
            <Text style={[styles.franchiseTitle, { color: Colors.primary }]}>₹25,000/year for all.</Text>
            <Text style={styles.franchiseSub}>Get onboarded as a service partner on Avasplot's ecosystem.</Text>
          </LinearGradient>
          ...
          >
            {mockOnboardRoles.map((fCard, index) => (
              <View key={fCard.id} style={[styles.franCard, { borderColor: fCard.theme.border, borderTopWidth: 4 }]}>
                ...
                <View style={styles.franPriceBox}>
                  ...
                  <Text style={styles.franSub}>per year (Excl. GST)</Text>
                </View>
          ...
          <View style={styles.whyPartnerContainer}>
            <Text style={styles.sectionHeading}>Why Onboard With Avasplot?</Text>
          ...
          <View style={styles.franFooterContainer}>
            <LinearGradient colors={['#0F172A', '#1E293B']} style={styles.franCtaBlock}>
              <Text style={styles.franCtaTitle}>Ready to Get Onboarded?</Text>
              <Text style={styles.franCtaSub}>Join India's fastest-growing land ecosystem platform — ₹25,000/year for everyone</Text>
              <TouchableOpacity style={styles.franEmailBtn}>
                <Text style={styles.franEmailTxt}>Get Started</Text>
              </TouchableOpacity>
            </LinearGradient>
          </View>
