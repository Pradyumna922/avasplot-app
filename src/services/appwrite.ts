...
// Utility: Test Appwrite connectivity
export async function testAppwritePing() {
  try {
    // The Appwrite JS SDK does not have a direct ping method, so we use a simple API call
    const health = await fetch("https://sgp.cloud.appwrite.io/v1/health");
    const data = await health.json();
    console.log("Appwrite Health:", data);
    return data;
  } catch (err) {
...
// Initialize Appwrite client with hardcoded config for direct connection
...
export const auth = {
  ...
  async loginWithGoogle() {
    // Use Appwrite's standard callback scheme for mobile OAuth redirects.
    // Format: appwrite-callback-[PROJECT_ID]://
    // This is automatically recognized by Appwrite and avoids the "Invalid URI"
    // error caused by Linking.createURL() producing triple-slash URIs.
    const callbackScheme = `appwrite-callback-${ENV.appwrite.projectId}`;
    const deepLink = `${callbackScheme}://`;

    console.log("[OAuth] Callback URL:", deepLink);

    // Manually construct the OAuth2 token URL.
    const oauthUrl =
      `${ENV.appwrite.endpoint}/account/tokens/oauth2/google` +
      `?project=${encodeURIComponent(ENV.appwrite.projectId)}` +
      `&success=${encodeURIComponent(deepLink)}` +
      `&failure=${encodeURIComponent(deepLink)}`;

    console.log("[OAuth] OAuth URL:", oauthUrl);

    // Open the OAuth URL in an in-app auth browser session
    const result = await WebBrowser.openAuthSessionAsync(oauthUrl, callbackScheme);
    ...
    console.log("[OAuth] userId:", userId ? "found" : "MISSING");
    console.log("[OAuth] secret:", secret ? "found" : "MISSING");
  ...
  async search(
    ...
    if (filters) {
      filteredDocs = filteredDocs.filter((doc) => {
        ...
        if (filters.minArea)
          matchesArea = matchesArea && doc.area >= filters.minArea;
        if (filters.maxArea)
          matchesArea = matchesArea && doc.area <= filters.maxArea;
      ...
      if (Platform.OS === "web") {
        ...
        try {
          ...
          if (sessionParams && sessionParams.providerAccessToken) {
            authHeaders = {
              "X-Appwrite-JWT": sessionParams.providerAccessToken,
            };
          }
        } catch {
...
export const users = {
  async getByPincode(pincode: string, role?: UserRole) {
    const queries = [Query.search("pincodes", pincode), Query.limit(25)];
    if (role) {
